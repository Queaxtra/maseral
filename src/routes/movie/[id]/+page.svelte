<script lang="ts">
    ...
    onMount(async () => {
        ...
        if (userLoggedIn) {
            ...
            isFavorited = await isMovieFavorited(movieId, userId);
        }
    });
    ...
    $: {
        if ($page.params.id) {
            ...
            ]).then(([movieData, similarData, actorsData]) => {
                movie = movieData;
...
<div transition:fade>
    <div class="relative h-[70vh] w-full">
        ...
        <div class="absolute bottom-0 left-0 right-0 p-8 max-w-7xl mx-auto">
            ...
            <div class="flex items-center gap-4 text-sm mb-4">
                ...
                <span>•</span>
                <span><i class="ri-heart-fill"></i> {movie.vote_average.toFixed(1)}</span>
                <span>•</span>
            ...
            <p class="text-lg opacity-80 max-w-3xl mb-6">{movie.overview}</p>
    ...
    <div class="max-w-7xl mx-auto py-12 px-4">
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div>
                <h2 class="text-2xl font-bold mb-4">Movie Details</h2>
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-white">{movie.runtime} minutes</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <p class="text-white">${movie.revenue.toLocaleString()}</p>
                    </div>
                </div>
            </div>
    ...
    <div class="max-w-7xl mx-auto py-12 px-4">
        <div class="flex items-center justify-between mb-6">
            ...
            <div class="flex gap-4">
                <button on:click={prevPage} class="px-4 py-2 bg-cGray rounded-lg disabled:opacity-50 border-white/10 border" disabled={currentPage === 1}><i class="ri-arrow-left-line"></i> Previous</button>
                <span class="px-4 py-2">Page {currentPage}</span>
                <button on:click={nextPage} class="px-4 py-2 bg-cGray rounded-lg border-white/10 border">Next <i class="ri-arrow-right-line"></i></button>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
            ...
            <div class="bg-cGray rounded-lg overflow-hidden cursor-pointer" on:click={() => goto(`/movie/${similar.id}`)}>
                ...
                <div class="p-4">
                    <h3 class="font-bold text-sm line-clamp-1">{similar.title}</h3>
    ...
    <div class="max-w-7xl mx-auto py-12 px-4">
        <h2 class="text-2xl font-bold mb-6">Watch Movie</h2>
        <div class="aspect-video rounded-lg overflow-hidden">
            <iframe class="w-full h-full" src={`https://vidsrc.to/embed/movie/${movie.id}`} frameborder="0" allowfullscreen></iframe>
        </div>
    </div>
</div>
{:else}
